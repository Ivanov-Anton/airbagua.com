...
$selected_folders = explode(',', $params->get('include_minify_folders', 'components/com_content,templates'));
?>
<div>
    <div class="pull-left">
        <div id="jao"></div>
...
<script>
    ...
    jQuery(document).ready(function ($) {
        ...
        jSelectFolders = function () {
            ...
            $.ajax({
                url: 'index.php?option=com_speedcache&task=connector.setFolders',
                type: "POST",
                data: data
            }).done(function (result) {
                window.parent.jModalClose();
            });
            window.parent.document.getElementById('jform_include_minify_folders').value = fchecked.join(',');
            window.parent.document.getElementById('jform_include_minify_folders_id').value = fchecked.join(',');
        }
        $('#jao').jaofiletree({
            script: 'index.php?option=com_speedcache&task=connector.listdir&tmpl=component',
            usecheckboxes: true,
            showroot: '/',
            oncheck: function (elem, checked, type, file) {
                var dir = file;
                var alldir = [];
                $('ul.jaofiletree li input[type="checkbox"]').each(function(i,v)
                {
                    if(i != 0){
                        var f = $(v).data('file');
                ...
                if (checked) {
                    if (file != "" && curFolders.indexOf(file) == -1) {
                        ...
                        array = curFolders.concat(alldir);
                        var a = array.concat();
