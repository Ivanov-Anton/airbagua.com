...
// Construct the entire class
$completeclass = ($gal_singlethumbmode) ? ' small-block-grid-1' : ' small-block-grid-2 medium-block-grid-3 '.$layoutclass;

?>

<ul id="sigProId<?php echo $gal_id; ?>" class="sigProContainer sigProResponsiveBlue<?php echo $completeclass.$singleThumbClass.$extraWrapperClass; ?>"<?php if($gal_singlethumbmode) echo ' style="min-width:'.$gallery[0]->width.'px;"' ?>>
    <?php foreach($gallery as $count=>$photo): ?>
    <li class="sigProThumb"<?php if($gal_singlethumbmode && $count>0) echo ' style="display:none !important;"'; ?>>
        <span class="sigProLinkOuterWrapper">
            <span class="sigProLinkWrapper">
                <a href="<?php echo $photo->sourceImageFilePath; ?>" class="sigProLink<?php echo $extraClass; ?>" style="width:100%;padding-bottom:<?php echo ($photo->height / $photo->width * 100); ?>%;" rel="<?php echo $relName; ?>[gallery<?php echo $gal_id; ?>]" title="<?php echo $photo->captionDescription.$photo->downloadLink.$modulePosition; ?>" data-fresco-caption="<?php echo $photo->captionDescription.$photo->downloadLink.$modulePosition; ?>" target="_blank" data-thumb="<?php echo $photo->thumbImageFilePath; ?>"<?php echo $customLinkAttributes; ?>>
