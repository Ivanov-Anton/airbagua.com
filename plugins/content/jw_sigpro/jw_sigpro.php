...
class plgContentJw_sigpro extends JPlugin
{
    ...
    // The main function
    public function renderSIGP(&$row, &$params, $page = 0)
    {
        ...
        $jQueryHandling = $pluginParams->get('jQueryHandling', 'jsdelivr');
        ...
        $cache_expire_time = $pluginParams->get('cache_expire_time', 720) * 60; // Cache expiration time in minutes
        // Advanced
        ...
        if (preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
            ...
            foreach ($matches[0] as $key => $match) {
                ...
                if (strpos($tagcontent, 'www.flickr.com') === false) {
                    ...
                    // Backwards compatibility for SIGE image galleries
                    if (strpos($tagcontent, ',') !== false) {
                        $tagContentParts = explode(',', $tagcontent);
                        $tagparams = array($tagContentParts[0]);
                    }
                    ...
                    // Get the Flickr set
                    /* example tag: {gallery}https://www.flickr.com/photos/joomladaygreece/albums/72157634594976836:20:200:80:1:2:jquery_colorbox:Galleria{/gallery} */

                    if (substr($tagcontent, 0, 4) != 'http') {
                        $tagcontent = 'https://'.$tagcontent;
                    }
                    ...
                    if (preg_match_all($flickrRegex, $flickrSetUrl, $flickrMatches, PREG_PATTERN_ORDER) > 0) {
                        ...
                        foreach ($galleryData as $key => $photo) {
                            ...
                            if ($gal_captions == 2) {
                                ...
                                $gallery[$key]->captionDescription = $photo->description.' - '.JText::_('JW_SIGP_LABELS_11').' <a href="'.$flickrSetUrl.'">'.$flickrSetTitle.'</a> '.JText::_('JW_SIGP_LABELS_12').' <a target="_blank" href="https://www.flickr.com/photos/'.$flickrUsername.'">'.$flickrUsername.'</a>';
                            } elseif ($gal_captions == 1) {
                                $gallery[$key]->captionTitle = JText::_('JW_SIGP_LABELS_09');
                                $gallery[$key]->captionDescription = JText::_('JW_SIGP_LABELS_10').' <a target="_blank" href="'.$flickrSetUrl.'">'.$flickrSetTitle.'</a> '.JText::_('JW_SIGP_LABELS_12').' <a target="_blank" href="https://www.flickr.com/photos/'.$flickrUsername.'">'.$flickrUsername.'</a>';
                            } else {
                ...
                if (JRequest::getCmd('format') == '' || JRequest::getCmd('format') == 'html') {
                    ...
                    if (version_compare(JVERSION, '3.0.0', 'ge')) {
                        ...
                        if (strpos($jQueryHandling, '1.')!==false) {
                            $jQueryHandling = 'googlecdn';
                        } // Fallback fix for SIGPro versions before 3.0.7
                        if ($jQueryHandling) {
                            ...
                            $jQueryLocation = array(
                                ...
                                'jsdelivr'   => 'https://cdn.jsdelivr.net/npm/jquery@'.$jQueryMinorRelease.'/dist/jquery.min.js'
                            );
